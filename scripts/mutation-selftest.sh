...
# number. thin's report.json (untracked, generated by the run below) holds
# four escaped mutants, all in calc.go. The hash below was computed once by
# hand with python3 while writing this case, per the spec:
#
#   python3 -c "
#   import json, hashlib
...
#
# A dedicated real run captures thin's report and copies it into $TMP, so
# this case and the escapedCount case below read their own copy rather than
# whichever report.json an earlier or later case happened to leave under
# scripts/testdata/thin -- that file is untracked, and case order must not
# be what proves this.
"$GATE" --root "$DATA/thin" --baseline "$TMP/floor-zero.tsv" \
  --survivors "$TMP/thin-survivors-ok.tsv" >/dev/null 2>&1
cp "$DATA/thin/report.json" "$TMP/thin-report.json"

expect_out 0 "the key of a known diff" \
  "calc.go:branch/if:48430df276c3b853" -- \
  "$GATE" --print-keys "$TMP/thin-report.json"
...
# escapedCount lying about .escaped's own length: a copy of thin's real
# report, corrupted with jq, must be refused rather than trusted -- the same
# reason check_report_trustworthy exists at all.
jq '.stats.escapedCount = 999' "$TMP/thin-report.json" > "$TMP/bad-escapedcount-report.json"
expect_out 2 "a report whose escapedCount does not match .escaped" \
  "escapedCount" -- \
  "$GATE" --print-keys "$TMP/bad-escapedcount-report.json"
...
# The multiset case: TWO identical rows for ONE surviving mutant, every other
# row correct. A comparison by SET (membership only) sees {branch/if,
# comparison, decrementer, incrementer} on both sides and passes -- wrong,
# because one of the two branch/if rows names a mutant that escaped only
# once. Only a comparison that counts repeats (a queue, not a set) can tell
# this apart from "every row has a mutant".
printf '# package\tkey\twhere\treason\n./\tcalc.go:branch/if:48430df276c3b853\tcalc.go:5 branch/if\tfixture: duplicated on purpose, to prove the comparison counts\n./\tcalc.go:branch/if:48430df276c3b853\tcalc.go:5 branch/if\tfixture: duplicated on purpose, to prove the comparison counts\n./\tcalc.go:expression/comparison:64a6ebd00ee15936\tcalc.go:4 expression/comparison\tfixture: matched on purpose\n./\tcalc.go:numbers/decrementer:b8c36547feae51db\tcalc.go:4 numbers/decrementer\tfixture: matched on purpose\n./\tcalc.go:numbers/incrementer:244ea632f3f1580d\tcalc.go:4 numbers/incrementer\tfixture: matched on purpose\n' \
  > "$TMP/surv-duplicate-row.tsv"
expect_out 1 "two identical rows for one surviving mutant" \
  "STALE ROW" -- \
  "$GATE" --root "$DATA/thin" --baseline "$TMP/floor-zero.tsv" \
    --survivors "$TMP/surv-duplicate-row.tsv"
...
# A row for a package the baseline does not list: caught once, reading the
# whole survivors file, before any package is measured.
printf '# package\tkey\twhere\treason\n./nonexistent/\tcalc.go:branch/if:48430df276c3b853\tcalc.go:5 branch/if\tfixture: package the baseline does not list, on purpose\n' \
  > "$TMP/surv-unlisted-pkg.tsv"
expect_out 2 "a row whose package the baseline does not list" \
  "./nonexistent/" -- \
  "$GATE" --root "$DATA/thin" --baseline "$TMP/floor-zero.tsv" \
    --survivors "$TMP/surv-unlisted-pkg.tsv"

# A row whose reason still starts with TODO: the ready-to-paste row the gate
# itself prints. A pasted row is not a read row, so the gate refuses it too.
printf '# package\tkey\twhere\treason\n./\tcalc.go:branch/if:48430df276c3b853\tcalc.go:5 branch/if\tTODO: read it\n' \
  > "$TMP/surv-todo-reason.tsv"
expect_out 2 "a row whose reason still says TODO" \
  "TODO" -- \
  "$GATE" --root "$DATA/thin" --baseline "$TMP/floor-zero.tsv" \
    --survivors "$TMP/surv-todo-reason.tsv"

expect_out 2 "the survivors file is missing" \
  "no-such-survivors.tsv" -- \
  "$GATE" --root "$DATA/thin" --baseline "$TMP/floor-zero.tsv" \
